...
def get_ai_suggestion(locator, html_source):
    prompt = (
        ...
        f"{html_source}\n\n"
        f"Using the provided HTML, suggest a simple, reliable, and valid XPath for identifying the element based on a combination of its attributes (e.g., `id` and `class` or `name` and `type`). "
        f"Ensure the XPath is in the format: `//*[@id='value' and contains(@class, 'class_name')]` or a valid equivalent. "
